...
40.7 **Audio Charts, Library Filter & Bitrate Fixes (Phase 40.7)**:
   - **Audio Charts ("UnknownUnknown") Fix**: `DeepInsights.tsx` generated the pie charts by concatenating string logic `lang + codec`. When null/undefined, strings resolved to "UnknownUnknown". Restructured codec checking logic to default to an empty string instead of "Unknown" strictly for codecs.
   - **Dashboard Library Filter**: Extracted `libraries` list using a `prisma.media.findMany({ distinct: ['libraryName'] })` call. Passed to a new interactive `<LibraryFilter>` component located directly inside the `DashboardHeader` next to the `TimeRangeSelector`. Dropdown updates the `excludeLibs` search param correctly rendering it client/server side on the fly alongside DB global excluded lists.
   - **Audio Bitrate Computation**: Changed `LibraryStats` logic inside `/media` index to extract both `track.size` and `track.durationMs` iteratively instead of standard PlaybackHistory. Calculated the bps mathematically: `(size * 8) / (durationMs / 1000)` mapping into kbps. Rendering inline replacing visual instances of "4K UHD" badging for albums specifically substituting with "x KBPS".
   - **Library Expand Typescript Fix & "Autre"/Other Removal**: Fixed `LibraryDetails` TypeScript Interface `lastAdded?: Date | null` to correctly support string `Date | string | null` generated by SSR. Applied a filtering loop (`validLibraries`) stripping out any libraries that have 0 combined total items (movies, series, music, or books). This prevented the "Autre" ghost library.
   - **Organic Aesthetic Reduction**: Subtly reduced hyper-animated glowing gradients, `.hover:scale-105`, and colored borders within the `LibraryStats` detail component to provide a professional layout preventing it from feeling over-designed ("too AI generated").

40.8 **Non-Admin Wrapped UX, Analytics Reformating (Phase 40.8)**:
   - **Sidebar Wrapped Button Refactor**: Prevented 404 errors for non-administrators trying to open `/wrapped/[id]` when the Wrapped feature globally disabled. Integrated `GlobalSettings` fetch inside `RootLayout` mapping `wrappedPeriodEnabled` boundary frame and passing down boolean metric `isWrappedVisible` securely into `Sidebar.tsx`.
   - **Top 10 DeepStats Episode Bug**: Patched a massive statistical data flaw where "Analyse Profonde" inside the Dashboard fetched all media content generically causing 1 TV Episode to equal 1 point for a director/actor. Restricting query inside `/api/stats/deep/route.ts` to `{ type: { in: ['Movie', 'Series'] } }` effectively stops episode inflation, ranking genuine unique franchises accurately.
   - **DeepStats UI Redesign**: Reformatted JSON text inside `StatsDeepAnalysis.tsx` to display human-readable syntax strings (`12 films/séries réalisés` instead of raw counts), extracting raw numbers from circular badges for a sleeker sentence flow.
   - **Library Hard-Coded Eradication**: Translated visual UI elements from `Bibliothèques` to `Détails par Collection` inside `LibraryStats.tsx`. Added specific loop check inside `app/media/page.tsx` forcibly dropping `name === tc('other')` libraries permanently from array population. 

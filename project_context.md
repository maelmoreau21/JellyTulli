...
12. **Critical Bug Fixes (Phase 11)**:
   - **Jellystat Import 174MB Fix**: `bodySizeLimit: '500mb'` only applies to Server Actions, not Route Handlers. The entire Jellystat import logic was moved to a **Server Action** (`src/app/actions/import-jellystat.ts`) using `'use server'`. The client now calls `importJellystatAction(formData)` directly instead of `fetch('/api/...')`. The Server Action receives the file via FormData (respecting the 500MB limit), then streams it through `stream-json` for incremental parsing. The old Route Handler is kept as fallback but is no longer called by the UI.

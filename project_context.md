...
9. **Build Standalone** : Le projet inclut un build Docker multi-stage minimaliste (`.next/standalone`) déployable d'un clic, tolérant aux erreurs TS/EsLint (idéal pour un Raspberry Pi) et avec migration automatisée (`docker-entrypoint.sh` + Prisma CLI global).
